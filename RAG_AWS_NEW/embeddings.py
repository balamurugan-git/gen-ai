...
from config import BEDROCK_REGION, BEDROCK_EMBEDDING_MODEL_ID, EMBEDDING_DIMENSIONS, BEDROCK_AWS_PROFILE
...
MAX_RETRIES = 5
RETRY_BACKOFF_SECONDS = 10  # linear: 10s, 20s, 30s, 40s, 50s
...
class BedrockEmbeddings:
    ...
    def __init__(self):
        ...
        # Use a dedicated AWS profile for Bedrock if configured (cross-account support)
        if BEDROCK_AWS_PROFILE:
            session = boto3.Session(profile_name=BEDROCK_AWS_PROFILE)
            logger.info(f"BedrockEmbeddings using AWS profile '{BEDROCK_AWS_PROFILE}' for Bedrock")
        else:
    ...
    def embed_text(self, text: str, input_type: str = "search_document") -> list[float]:
        ...
        if len(text) > MAX_CHARS_PER_CALL:
            logger.warning(f"Text truncated: {len(text)} → {MAX_CHARS_PER_CALL} chars")
